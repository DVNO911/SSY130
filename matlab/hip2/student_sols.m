...
% FIR-filter using firpm
    function h = gen_filter()
        fs = 1;
        filter_order = 60;
        f = [0 0.05 0.1 fs/2]/(fs/2);
        a = [0 1 0 0].*f*pi;
        h = firpm(filter_order, f, a, 'differentiator'); %TODO: This line is missing some code!
        % Plotting
        figure(10)
        [H,w] = freqz(h,1,250);
        plot(f,a, w/pi,abs(H));
        xlabel('f/(f_s/2)');
        ylabel('|H|');
        title('Ideal filter compared to filter generated by firpm with n = 60');
        legend('Ideal', 'FIR')
    end

% Euler filter
    function h = gen_euler()
        fs = 1;
        delta_t = 1/fs;
        h = zeros(1,2);
        h(1) = 1/delta_t;
        ...
        xlabel('f/(f_s/2)');
        ylabel('|H|');
        title('Ideal filter compared to Euler filter');
        legend('Ideal', 'Euler')
    end


funs.gen_filter = @gen_filter;
funs.gen_euler = @gen_euler;
